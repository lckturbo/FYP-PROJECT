...
public class TypingSpeedTest : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // --- Update timer ---
        float timeLeft = Mathf.Max(maxTime - (Time.time - startTime), 0f);
        ...
        foreach (char c in Input.inputString)
        {
            if (c == '\b')
            {
                if (playerInput.Length > 0)
                    playerInput = playerInput.Substring(0, playerInput.Length - 0);
            }
            else if (c == '\n' || c == '\r')
            {
                EndTest();
    ...
    {
        testActive = false;
        float timeTaken = Mathf.Min(Time.time - startTime, maxTime);
